...
- User structure 使用结构：是一种专门的依赖关系，用来**拓展或缩小系统**，创建子系统
- layer structure 层结构：通过**接口**提供一个有凝聚力的服务集合
...
## 组件-连接件框架 Component-and-Connector Pattern
...
- 成本：取决于建模参数，如arrival rate of events, chosen queuing discipline, chosen scheduling algorithm, network topology, network bandwidth, routing algorithm chosen, service time for events
- 作用：估计延迟时间
...
- 对于10KSLOC（代码行数）项目，甜蜜点在最左边。 花费大量时间进行前期工作对于一个小型项目来说是一种浪费
...
- 从需求文档收集
- 采访利益相关者，举行 QAW（Quality attribute workshop 质量属性研讨会）
- 通过理解业务目标获取
- 从效用树获取
...
- 分解：整个系统被分解成多个部分，每个部分承担部分的质量属性要求
