...
- Response：记录故障；通知各方，如用户或系统；根据规则禁止导致错误的源头
...
- Stimulus：一段完整代码的完成，如类、层
- Artifact：代码段、完整应用
- Environment：设计时、开发时、编译时、部署时
- Response：准备测试的环境、执行测试并捕获结果
- Response measure：准备测试的时间、执行测试的时间
...
- Source：**最终用户**
- Stimulus：学习系统特性、学会有效使用系统、适配系统
- Artifact：**系统**
- Environment：运行时、配置时
- Response：上下文相关的帮助系统；撤销操作；取消操作
- Response measure：任务时间、用户满意度、成功操作的比例
